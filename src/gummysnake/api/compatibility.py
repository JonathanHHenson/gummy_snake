...
from gummysnake.api import advanced as _advanced
...
def unsupported_feature(name: str, reason: str) -> None:
    raise UnsupportedFeatureError(f"{name} is not supported by Gummy Snake. {reason}")
...
def _deferred_offscreen_api(name: str) -> None:
    unsupported_feature(
        name,
        "Offscreen graphics and framebuffer APIs are deferred until gummy_canvas owns "
...
def _deferred_canvas_api(name: str) -> None:
    unsupported_feature(
        name,
        "This canvas API is deferred until gummy_canvas exposes the required native "
...
def create_color_picker(*_args, **_kwargs) -> None:
    unsupported_feature(
        "create_color_picker",
        "DOM form APIs are intentionally excluded; use native Python UI libraries "
        "outside Gummy Snake.",
    )
...
def load_xml(*_args, **_kwargs) -> None:
    unsupported_feature("load_xml", "XML is intentionally excluded.")


def load_table(*_args, **_kwargs) -> None:
    unsupported_feature(
        "load_table",
        "Table and TableRow are intentionally excluded.",
    )


def table_row(*_args, **_kwargs) -> None:
    unsupported_feature("table_row", "TableRow is intentionally excluded.")
...
def _deferred_sensor_api(name: str) -> None:
    unsupported_feature(
        name,
        "Device acceleration/orientation APIs are deferred until Gummy Snake has a native "
        "sensor provider.",
    )
...
def text_to_points(*_args, **_kwargs) -> None:
    unsupported_feature(
        "text_to_points",
        "Font outline/path helpers are deferred until gummy_canvas exposes deterministic "
...
def no_canvas(*_args, **_kwargs) -> None:
    unsupported_feature(
        "no_canvas",
        "Gummy Snake currently requires a gummy_canvas surface even for bounded/headless runs. "
...
def strands(*_args, **_kwargs) -> None:
    unsupported_feature(
        "strands",
        "strands browser/WebGPU semantics are intentionally excluded unless a native "
