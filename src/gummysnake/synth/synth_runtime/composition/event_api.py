...
from gummysnake.synth.synth_runtime.composition.builder_context import (
    _current_builder,
    _next_node_id,
)
...
if TYPE_CHECKING:
    from gummysnake.synth.synth_runtime.composition.context_managers import ThreadContext
    from gummysnake.synth.synth_runtime.composition.definitions import FxDefinition, SynthDefinition
...
def _compiled_synth_definition(name: str) -> CompiledSynthDefinition | None:
    ...
    with contextlib.suppress(ArgumentValidationError):
        from gummysnake.synth.synth_runtime.playback_export.track import builtin_synth_path
...
def _compiled_fx_definition(name: str) -> CompiledFxDefinition | None:
    ...
    with contextlib.suppress(ArgumentValidationError):
        from gummysnake.synth.synth_runtime.playback_export.track import builtin_fx_path
