...
class CanvasBackendRuntimeMixin:
    def run(self, sketch: Sketch, *, max_frames: int | None = None) -> None:
        ...
        should_run_interactive = _backend(self)._interactive or (
            ...
            and _backend(self).capabilities.interactive
    ...
    def _run_interactive(self, sketch: Sketch, *, max_frames: int | None = None) -> None:
        ...
        _backend(self)._open_interactive_window(canvas)
        _backend(self)._running = True
    ...
    def _interactive_sleep_delay(self, draw_pending: bool, interval: float) -> float:
        if draw_pending:
            return max(
                0.0,
                min(_backend(self)._next_frame_time - _backend(self)._perf_counter(), interval),
            )
    ...
    def _debug_interactive_tick(self, label: str, context: Any, drew_frame: bool) -> None:
        if not _backend(self)._debug:
            return
        _backend(self)._last_idle_debug_frame = None
        print(
            "[canvas-debug] "
    ...
    def _debug_interactive_idle(self, context: Any) -> None:
        if (
            not _backend(self)._debug
            or _backend(self)._last_idle_debug_frame == context.state.timing.frame_count
        ):
            return
        _backend(self)._last_idle_debug_frame = context.state.timing.frame_count
        print(
    ...
    def _draw_and_present(self, sketch: Sketch) -> bool:
        ...
        if before_frame_count is None or after_frame_count != before_frame_count:
            present_start = _backend(self)._perf_counter()
            self.present()
            present_end = _backend(self)._perf_counter()
            _backend(self)._record_pacing_duration("draw", draw_duration_ms)
            _backend(self)._record_pacing_duration(
                "present", (present_end - present_start) * 1000.0
            )
            _backend(self)._record_present_interval(present_end)
            return True
        return False
    ...
    def _advance_next_frame_time(self, now: float, interval: float) -> None:
        ...
        while _backend(self)._next_frame_time <= now:
            _backend(self)._next_frame_time += interval
