...
canvas_backend_perf_scenes = import_module("canvas_backend_perf.scenes")
canvas_backend_perf_child = import_module("canvas_backend_perf.child")
...
def test_canvas_benchmark_stress_variants_are_registered(monkeypatch) -> None:
    ...
    fake = FakeGs()
    monkeypatch.setattr("canvas_backend_perf.scenes.gs", fake)
    ...
    monkeypatch.setattr("canvas_backend_perf.scenes.require_context", lambda: FakeContext())
    monkeypatch.setattr("canvas_backend_perf.scenes.sprites", [FakeSprite()])
...
def test_canvas_benchmark_setup_accepts_stress_variants(monkeypatch) -> None:
    ...
    monkeypatch.setattr("canvas_backend_perf.scenes._sprite", lambda *_args: FakeSprite())
    monkeypatch.setattr("canvas_backend_perf.scenes._reset_asteroids", lambda: None)
