...
EXAMPLE_ROOT = ROOT / "examples" / "11_temporary_perf_tests"
...
def _load_scene(name: str, path: Path, frames: int, headless: bool) -> ModuleType:
    ...
    module_name = f"temporary_ecs_perf_scene_{name}"
    spec = importlib.util.spec_from_file_location(module_name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load temporary ECS scene {name!r} from {path}")
...
def main(argv: list[str]) -> None:
    if len(argv) != 4:
        raise SystemExit(
            "usage: temporary_ecs_perf_child.py <scene> <frames> <headless|interactive>"
        )
    scene_name = argv[1]
    frames = int(argv[2])
    mode = argv[3]
    if scene_name not in SCENES:
        raise ValueError(f"unknown temporary ECS benchmark scene: {scene_name}")
