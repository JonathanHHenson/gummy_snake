...
class _ScenesModule(Protocol):
    def draw_scene(self, variant: str) -> None: ...

    def setup_scene(self, variant: str) -> None: ...


def _load_scenes_module() -> _ScenesModule:
    if __package__ in {None, ""}:
        sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    return cast("_ScenesModule", import_module("canvas_backend_perf.scenes"))


_scenes = _load_scenes_module()
draw_scene = _scenes.draw_scene
setup_scene = _scenes.setup_scene
...
def _warm_up_scene(variant: str) -> None:
    """Populate renderer pipelines and caches outside the timed frame window."""
