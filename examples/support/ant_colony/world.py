"""Deterministic world seeding and pheromone-plan building for ant colonies."""
...
EntityAdder = Callable[..., object]


def _add_voxel(
    add_entity: EntityAdder, cell: tuple[int, int], *components: Any, tags: list[str]
) -> None:
    x, y = _cell_center(cell)
    add_entity(GridVoxel(float(cell[0]), float(cell[1]), x, y), *components, tags=tags)


def _add_pheromone_voxels(
    add_entity: EntityAdder,
    ...
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            cell = (x, y)
            if cell in walls or (
                cell not in required_cells
                and (x % PHEROMONE_STRIDE != 0 or y % PHEROMONE_STRIDE != 0)
            ):
                continue
...
def _seed_ants(add_entity: EntityAdder, center: tuple[int, int], tag: str, *, seed: int) -> None:
...
def populate_world(add_entity: EntityAdder) -> dict[str, int]:
    """Populate an ECS entity sink with the fixed ant-colony world and return counts."""
    walls = _wall_voxels()
    foods = _food_voxels()
    red_hill_cells = _hill_voxels(RED_HILL)
    blue_hill_cells = _hill_voxels(BLUE_HILL)
    for cell in sorted(walls):
        _add_voxel(add_entity, cell, WallVoxel(1.0), tags=[WALL_TAG])
    for cell in sorted(foods):
        _add_voxel(add_entity, cell, FoodVoxel(1.0), tags=[FOOD_TAG])
    for cell in sorted(red_hill_cells):
        _add_voxel(add_entity, cell, HillVoxel(0.0), tags=[RED_HILL_TAG])
    for cell in sorted(blue_hill_cells):
        _add_voxel(add_entity, cell, HillVoxel(1.0), tags=[BLUE_HILL_TAG])
    pheromone_voxels = _add_pheromone_voxels(add_entity, walls, red_hill_cells, blue_hill_cells)
    _seed_ants(add_entity, RED_HILL, RED_ANT_TAG, seed=11)
    _seed_ants(add_entity, BLUE_HILL, BLUE_ANT_TAG, seed=29)
    return {
...
def update_pheromone_query(marker: ecs.Query, ant: ecs.Query, *, red_colony: bool) -> None:
    """Append deterministic pheromone aggregation actions to the active ECS plan."""
