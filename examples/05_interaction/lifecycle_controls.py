...
def draw() -> None:
    ...
    if p5.frame_count() == 8 and not AUTO_PAUSED and ARGS.headless is False:
        AUTO_PAUSED = True
        p5.no_loop()
    ...
    p5.fill(240)
    p5.text_size(16)
    p5.text(f"canvas: {p5.width()} x {p5.height()}", 30, 44)
    p5.text(f"frame: {p5.frame_count()}  draw calls: {DRAWS}", 30, 72)
    p5.text(f"looping: {p5.is_looping()}  manual redraws: {MANUAL_REDRAWS}", 30, 100)
    p5.text("Auto-pauses at frame 8. Click redraws once. Press L to loop, P to pause.", 30, 130)

    p5.no_stroke()
    for i in range(p5.frame_count() % 12 + 1):
...
def key_pressed(event) -> None:
    pressed = event.key.lower() if event.key else ""
    if pressed == "l":
        p5.loop()
    if pressed == "p":
        p5.no_loop()
