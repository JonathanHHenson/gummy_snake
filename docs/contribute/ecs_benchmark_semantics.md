...
fingerprint and commit. CI runs catalog/schema validation, deterministic world/frame
oracles, path assertions, and headless smoke as correctness coverage. Native-interactive
runs are optional manual information.
...
are correctness-only and do not write timing history. Native-interactive and direct-Rust
routes report their actual availability and remain optional informational runs.
...
- mixed ordered drawing remains incomplete. Native input, native interactive presentation,
  frame pacing, and canvas/ECS/synth contention are optional manual informational suites.
  A native-interactive request fails closed with the declared route error and cannot
  downgrade to headless;
...
Comparable release timing uses the local-first CLI: run `worktree` for a non-writing
comparison, then `record-head` from the clean release commit. Records are stored under
`.scratch/benchmark/history`, keyed by exact fingerprint and commit. Use `list` to inspect
records and `audit` to validate the store; pass `--history <path>` before the subcommand
for another local directory. Release builds, fixed workloads, deterministic oracles, raw
samples, path diagnostics, and the fixed >5% policy remain required. `R` and `I` are
optional informational routes and report their actual availability.
