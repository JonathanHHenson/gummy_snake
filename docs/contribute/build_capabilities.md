...
| GPU renderer | Optional/platform-dependent | `wgpu` path in `gummy_canvas` | `gummysnake.rust.canvas.canvas_gpu_status()` and `CanvasBackend.gpu_status()` | `uv run python examples/01_getting_started/basic_shapes.py --headless --frames 1` |
...
| Synth runtime | Required for `gummysnake.synth` rendering, sample decoding, FX execution, and bounded offline workers | `crates/gummy_synth` linked into `crates/gummy_canvas`, exposed through `gummysnake.rust._canvas`; validated pure-Rust compile/render/decode/WAV calls release the GIL | `gummysnake.rust.canvas.require_canvas_runtime()` validates canvas ABI 21 and synth bridge functions; `sy.configure_workers(...)` and `sy.synth_diagnostics()` expose worker/GIL/cache/block/session/queue metadata | `cargo test --manifest-path crates/gummy_synth/Cargo.toml` plus `uv run pytest tests/unit/synth/test_synth_tracks.py -q` |
| Native audio playback | Optional at runtime but the only supported playback path | One process-local SDL3 device manager in `gummy_canvas` mixes loaded/rendered `CanvasSound` voices and synth sessions at explicit 48 kHz stereo 16-bit PCM | `Sound.play()`/`Track.play()` fail clearly when SDL cannot open a device; `sy.synth_diagnostics()` exposes device/voice/session/queue state | Deterministic sound/synth tests plus optional maintainer-invoked physical-device checks |
...
the wheel through an isolated `uv` consumer environment, requires canvas ABI 21
and ECS ABI 6, validates health checks, exercises an empty Rust ECS world and a
