...
| ECS public and logical-plan layer | `src/gummysnake/ecs/` | `src/gummysnake/ecs/world.py` is a compatibility facade; `src/gummysnake/ecs/world_facade/` owns the Python world implementation | `tests/unit/test_ecs_core.py`, `test_ecs_plans.py`, `test_ecs_schedule.py`, `test_ecs_spatial.py` |
| ECS expressions and spatial relations | `src/gummysnake/ecs/expressions/`, `src/gummysnake/ecs/spatial/` | `src/gummysnake/ecs/actions.py`, `systems.py`, and `physical.py` are public composition surfaces | ECS plan and spatial tests |
